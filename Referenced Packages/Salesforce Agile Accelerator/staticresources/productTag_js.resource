...
ProductTag.prototype = {
    ...
    render: function() {
        ...
        $('<div id="productTagItem" class="productTagItem curvedContainer" aria-label="Product Tag" style="display:none" tabindex="0"><span class="productTagItemBody"><span class="productTagItemTeam"></span> <span class="productTagItemName curvedContainer"></span></span><a class="productTagItemRemove" alt="Remove" aria-label="Remove Product Tag" title="Remove Product Tag" tabindex="0">&nbsp;</a></div>')
            .prependTo(this.$el);
        ...
        this.$('.productTagInput')
            ...
            .autocomplete({
                ...
                focus: function(event, ui) {
                    ...
                    //$(this).val(ui.item.agf__Team__r.Name + ' > ' + ui.item.Name);
                    return false;
                },
                ...
                select: function(event, ui) {
                    ...
                    //return $('<li><a></a></li>').find('a').text(item.agf__Team__r.Name + ' > ' + item.Name).end().data("item.autocomplete", item).appendTo(ul);
                    return $("<li>").attr('role','option').attr('id','selection-' + item.Id).attr("data-value", item.agf__Team__r.Name + ' > ' + item.Name).append(item.agf__Team__r.Name + ' > ' + item.Name).appendTo(ul);
                }
    },
